...
const BuyNumbers = () => {
  ...
  useEffect(() => {
    if (selectedNumberType) {
      if (typeNeedsCountry(selectedNumberType)) {
        // Reset country to ensure user picks a relevant one for the new service type
        // and prevent immediate service fetching for the default country
        setSelectedCountry({ name: "", code: "", value: "", id: null });
      ...
      <div className="mb-6">
        ...
        {selectedNumberType?.notice ? (
          <p className="text-yellow-600 font-bold text-sm md:text-base bg-yellow-50 border border-red-200 rounded p-2 inline-block">
            âš ï¸ NOTICE: {selectedNumberType.notice}
          </p>
        ) : (
                      ...
                      {hasMultipleCosts ? (
                        ...
                        <h3 className="text-primary font-semibold">
                          {typeof service.cost === "number"
                            ...
                            : `â‚¦${service.cost ? String(service.cost).replace(/^â‚¦/, '').replace(/^N/, '').trim() : "0.00"}`}
                        </h3>
                      )}
                    </div>
                    <button
